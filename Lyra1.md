...
源码分析思路 ：GAS代码复杂，所以只能从公开的接口追溯底层源码，尤其是AbilityTask这一块，<br>
Task是对GAS底层的封装，重要的同时 也具有参考性，所以Task的分析过程会更详细. 通过Task理解底层逻辑.

GameplayAbility: 负责逻辑流（例如：播放动画、生成火球）.它不存储数值，只管“执行过程”.<br>
AttributeSet: 专门存储数值数据（例如：生命值、法力值）.它与逻辑分离，保证数据的唯一来源.<br>
GameplayEffect: 负责修改数据.技能不直接修改生命值，而是通过发送一个 GE 来“申请”修改.这使得所有的`Buff` `Debuff`都能以统一的方式被追踪和撤销.<br>


状态即标签： 通过 `GameplayTag` 来描述当前状态.<br>
逻辑过滤： 技能可以声明“拥有某标签时不可启动”或“启动时给自身添加某标签”.这种基于标签的逻辑使得不同技能之间的交互（如：沉默状态下禁止施法）变得极为简单且易于扩展.<br>

客户端先行：客户端可以立即触发技能逻辑（如播放动画），而无需等待服务器回传.<br>
服务器校验：服务器负责最终的数值计算和状态验证.<br>
自动同步：GAS 自动处理属性的同步、技能的激活确认以及效果的复制.<br>
...
这个过程中，`ASC`是一个消息传输的总线，各个部件通过`ASC`实现数据交流 而无需耦合.
...
`GE Specs`:<br>
`GE Specs`是`GE`的运行时版本.您可以将它们视为围绕`GE`（`GE`是资产）的实例化数据包装器.<br>
因此，蓝图功能更关注的是`GE Specs`而非`GE`本身，这体现在 `AbilitySystemBlueprintLibrary` 中.


GameplayEffectSpc 提供以下信息：
- 使用哪个 UGameplayEffect（const data）
- 什么等级
- 由谁发起

`FGameplayEffectSpec` 是可修改的，我们从初始条件开始，并对其应用修改.<br>
从这个意义上说，Spec是有状态/可变的，但它仍然不同于 `FActiveGameplayEffect`，<br>
后者是 `FGameplayEffectSpec` 的一个已应用实例.
...
```cpp
FActiveGameplayEffectHandle UAbilitySystemComponent::BP_ApplyGameplayEffectToSelf(TSubclassOf<UGameplayEffect> GameplayEffectClass, float Level, 
...
```
传进来一个GE的类，这个函数会在Context无效时 创建一个Context.<br>
...
`FGameplayEffectContextHandle` 拥有网络复制功能，具体的 `NetSerialize`分析 可以参考后文的`TargetData`.



```cpp
virtual FGameplayEffectContext* UAbilitySystemGlobals::AllocGameplayEffectContext() const
{
	return new FGameplayEffectContext();
}

UFUNCTION(BlueprintCallable, Category = GameplayEffects)
virtual FGameplayEffectContextHandle UAbilitySystemComponent::MakeEffectContext() const
{
...
```
从Global里面创建一个Context类，`AllocGameplayEffectContext` 是virtual函数，可以自定义扩展 返回自定义的Context类.

`MakeEffectContext` 是virtual函数，可以自定义扩展，添加其他信息.<br>

默认情况下，只在 `Context` 里面添加了来源Actor，<br>
`Context.AddInstigator` : `Context` 会保存`OwnerActor` `AvatarActor` 以及 `OwnerActor`里面的ASC.<br>
对应信息:<br>
`OwnerActor` - `Instigator`<br>
`AvatarActor` - `EffectCauser`<br>
`InstigatorAbilitySystemComponent`.

```
Instigator is who owns the ability that spawned this, 
EffectCauser is the actor that is the physical source of the effect, such as a weapon. 
They can be the same. 
```

---

Context创建之后就可以ApplyGE.
...
如果Actor是权威的，或者预测Key是有效的， 就可以ApplyGE.<br>

OwnerActor权威性:如果ASC的Owner是权威的，就说明Actor在服务器上，可以应用GE.<br>
服务器上的其他Actor 例如火焰区域可以造成伤害，区域在对ASC应用GE时，它是可以成功的，<br>
客户端的玩家对自己 或 其他玩家直接应用GE 会因为这个检测而无法应用GE. 客户端没有权威性.

预测Key有效:直接从ASC的`BP_ApplyGameplayEffectToSelf`函数 调用过来是没有预测Key的，参数是空值，<br>
GA里面的`ApplyGameplayEffect`一类函数 在应用GE时就带有预测Key，<br>

所以客户端玩家只能用GA对ASC应用一个GE，如果GA被服务器拒绝了，客户端也会撤销GE.
...
FActiveGameplayEffectHandle UAbilitySystemComponent::ApplyGameplayEffectSpecToSelf(const FGameplayEffectSpec &Spec, FPredictionKey PredictionKey)
{
    ...
    // 在添加操作完成后对容器进行作用域锁定，以防止新效果在添加操作的剩余过程中可能被意外破坏
...
`FScopedActiveGameplayEffectLock` <br>
提供一种机制，在可能回调游戏代码时锁定活跃的 GE 列表.<br>
例如，如果 `FActiveGameplayEffectsContainer` 中的某些内部代码正在遍历活跃 GE 列表<br>
或持有指向列表中某个元素的指针，对该列表的任何更改都可能导致内存从底层移出.<br>

此作用域锁会将删除和添加操作排队，直到作用域结束.添加和删除操作实际上会生效，<br>
但我们会将对活跃 GE 列表的内存操作延迟执行.<br>
...
```
`ImmunityGameplayEffectComponent` 依赖这个机制来实现免疫.
...
```
`Spec.Def->CanApply` 调用 GE组件的 `CanGameplayEffectApply`函数.<br>
`UChanceToApplyGameplayEffectComponent` | `UTargetTagRequirementsGameplayEffectComponent` 依赖这个机制决定GE是否要应用.

---

![alt text](Lyra1/img2/image-16.png)
接下来检查 `Modifiers` 的有效性. 检查GE要修改的`Attribute`是否有效:
```cpp
Handle UAbilitySystemComponent::ApplyGameplayEffectSpecToSelf(const FGameplayEffectSpec &Spec, FPredictionKey PredictionKey)
{
...
由于要处理Stack情况，所以 `AppliedEffect` 可能是新添加的GE，<br>
也可能是已经应用的GE 因为再次ApplyGE 导致已有的GE增加了StackCount，<br>

如果是已有的GE，那么 `ApplyGameplayEffectSpec` 会返回此前已应用的GE.
...
```
服务器上应用GE是非预测，直接执行`ExecuteGameplayEffect`，当开头的`ScopeLock`生命周期结束时 修改容器状态，<br>
因为容器状态发生变化，需要同步到客户端， 客户端在接收到消息时 会做一些操作.
...
服务器调用 `ApplyGameplayEffectSpecToSelf` 后，最终会在 `FActiveGameplayEffectsContainer::ApplyGameplayEffectSpec` 中创建一个 `FActiveGameplayEffect` 对象，并将其加入 `ActiveGameplayEffects` 容器.<br>
该容器是一个 `FFastArraySerializer`，通过 `NetDeltaSerialize` 自动将添加/修改/删除的 `FActiveGameplayEffect` 复制到客户端.

客户端在收到 `ActiveGameplayEffects` 之后，`FActiveGameplayEffect::PostReplicatedAdd` 没有执行任何如伤害计算、扣血等 `ExecuteGameplayEffect` 逻辑.<br>
它所做的一切都是为了承接状态：把 Buff 的数值（Modifiers）挂到本地用于 UI 和动作计算、把 Buff 的标签（Tags）塞进本地Map、并在本地产生对应的`GameplayCue`.
...
`OnAddedToActiveContainer` 判定GE可以激活，`bActive = true`，`!bActive`传入`InhibitActiveGameplayEffect`就是 `false`，<br>
不管能不能激活，先设`Effect.bIsInhibited = true`，抑制GE 让它不生效，处于休眠状态.
...
```
`ActiveGE->bIsInhibited = true`

`bActive = true`

此时，`ActiveGE->bIsInhibited = true` , `bInhibit = false` 两者不相等，<br>
将 `ActiveGE->bIsInhibited` 设为 false，表示不抑制，可以执行.<br>
由于 `bInhibit = false` ，所以 if执行false分支.

`bActive = false` ， 将`ActiveGE->bIsInhibited` 设为 true，不执行.
...
`ApplyGameplayEffectSpec` 里面添加了移除持续GE的定时器，在 `if (DurationBaseValue > 0.f)` 这一段里面.


下面是 `InhibitActiveGameplayEffect` 简化的代码 :
```cpp
void UAbilitySystemComponent::InhibitActiveGameplayEffect(FActiveGameplayEffectHandle ActiveGEHandle, bool bInhibit, bool bInvokeGameplayCueEvents)
{
...
```
`AggregatorOnDirtyBatcher` 在析构时 检查引用计数， 调用 `FAggregator::BroadcastOnDirty`.
...
void FActiveGameplayEffectsContainer::AddActiveGameplayEffectGrantedTagsAndModifiers(FActiveGameplayEffect& Effect, bool bInvokeGameplayCueEvents)
{
    if (Effect.Spec.GetPeriod() <= UGameplayEffect::NO_PERIOD)
    {
        for (int32 ModIdx = 0; ModIdx < Effect.Spec.Modifiers.Num(); ++ModIdx)
        {
            ...
            Aggregator->AddAggregatorMod(EvaluatedMagnitude, ModInfo.ModifierOp, ModInfo.EvaluationChannelSettings.GetEvaluationChannel(), 
            &ModInfo.SourceTags, &ModInfo.TargetTags, Effect.PredictionKey.WasLocallyGenerated(), Effect.Handle);
        }
    }
}
```

这里的 `Aggregator->AddAggregatorMod` 不会调用`BroadcastOnDirty`，

原因:<br>
`AggregatorOnDirtyBatcher` 构造时，`GlobalBatchCount++`，<br>
`FindOrCreateAttributeAggregator` 会绑定`OnDirty`委托.

这两个条件的叠加 导致 `BroadcastOnDirty` 只添加`DirtyAggregators`.<br>
```cpp
...
```
值的来历 : 从CDO的Modifer里面计算值 并保存在ModSpec.
```cpp
FActiveGameplayEffect* FActiveGameplayEffectsContainer::ApplyGameplayEffectSpec(const FGameplayEffectSpec& Spec, FPredictionKey& InPredictionKey, bool& bFoundExistingStackableGE)
{
...
void FGameplayEffectSpec::CalculateModifierMagnitudes()
{
	for(int32 ModIdx = 0; ModIdx < Modifiers.Num(); ++ModIdx)
	{
        ...
        ModDef.ModifierMagnitude.AttemptCalculateMagnitude(*this, ModSpec.EvaluatedMagnitude);
    }
}
```
值计算的几个路径:
...
FActiveGameplayEffectsContainer::PredictivelyExecuteEffectSpec
->FGameplayEffectSpec::CalculateModifierMagnitudes
...
`UAbilitySystemComponent::InhibitActiveGameplayEffect` 执行完以后，`AggregatorOnDirtyBatcher` 触发析构.
...
```
此时 `BroadcastOnDirty` 判断`GlobalBatchCount > 0 ` 失败，所以会执行后续的内容，广播事件.

翻译自 `FAggregator::BroadcastOnDirty` 函数内的注释:

下面的代码试图避免递归问题：一个聚合器变脏，而当它向外广播时，又有其他操作把它再次弄脏.<br>
最坏的情况是循环的属性依赖：`MaxHealth` -> `MaxMana` -> `MaxHealth`.<br>
这个问题可能无法修复.我们应该更早地检测到它.<br>

不那么严重的情况是，在广播过程中，有人应用了一个 GE，又把该属性弄脏了.<br>
只要不是无限循环……应该没问题.<br>

现在，这段代码允许最多 `MAX_BROADCAST_DIRTY` 次递归调用.<br>
这个数字是随意设定的，我们试图提供一个不会因为错误数据而崩溃（无限循环）的解决方案，<br>
同时也不要在触发时破坏/导致 bug.<br>
...
当 `BroadcastOnDirty` 广播`OnDirty`事件时， `FActiveGameplayEffectsContainer::OnAttributeAggregatorDirty` 就会响应，以更新AttributeSet的值.
```cpp
/* bFromRecursiveCall 是false */
...
{
    // 我们的聚合器已发生变化，需要重新评估该聚合器并更新属性的当前值.
    // 注意，这不是一次执行（execution），因此 FAggregatorEvaluateParameters 中不需要填充 'source' 和 'target' 标签.
    // 那些具有所需自身标签（owned tags）的 ActiveGameplayEffect 会通过委托来开启/关闭，并在发生时自动向属性聚合器添加或移除自身.
...
```
`Aggregator->Evaluate` 使用BaseValue为基础值，计算CurrentValue.
```cpp
void FActiveGameplayEffectsContainer::InternalUpdateNumericalAttribute(FGameplayAttribute Attribute, float NewValue, 
const FGameplayEffectModCallbackData* ModData, bool bFromRecursiveCall)
{
	const float OldValue = Owner->GetNumericAttribute(Attribute);
	Owner->SetNumericAttribute_Internal(Attribute, NewValue);
}
...
`SetNumericAttribute_Internal` 会调用AttributeSet的AttributeChange函数 :
```cpp
OldValue = DataPtr->GetCurrentValue();
Dest->PreAttributeChange(*this, NewValue);
DataPtr->SetCurrentValue(NewValue);
...
`ApplyGameplayEffectSpec` 主要负责处理有持续时间的 GE，<br>
`PreGameplayEffectExecute` / `PostGameplayEffectExecute` 是专门为“瞬间生效并改变属性基础值”的 `Instant` 或 `Execution Calculation` 的GE设计的.

Instant GE：会永久修改属性的 `BaseValue`.

Duration / Infinite GE：不修改 Base Value，而是创建修饰器 `Modifiers` 注册到 `AttributeAggregatorM` 中，<br>
从而动态影响 `CurrentValue`.
...
从服务器视角来看，使用 `ExecuteGameplayEffect` 处理Instant GE. 客户端会因为网络同步而执行相关事情.
...
`ExecuteActiveEffectsFrom` 函数中有注释，分为几个部分 : `Modifiers` `Executions` `Invoke GameplayCue events`,

**Modifiers**<br>
遍历CDO的`Modifiers`，调用 `InternalExecuteMod` 修改Attribute.
```cpp
->UAttributeSet::PreGameplayEffectExecute

->FActiveGameplayEffectsContainer::ApplyModToAttribute
-->FActiveGameplayEffectsContainer::SetAttributeBaseValue

--->UAttributeSet::PreAttributeBaseChange
--->FGameplayAttributeData::SetBaseValue

--->FAggregator::GetBaseValue
--->FAggregator::SetBaseValue

->UAttributeSet::PostGameplayEffectExecute(ExecuteData);
```
`FAggregator::SetBaseValue` 在修改BaseValue时，广播 `BroadcastOnDirty`.

**Executions**<br>

![alt text](Lyra1/img2/image-74.png)

执行 `UGameplayEffectExecutionCalculation` 的 `Execute`函数，应用返回的属性结果,<br>
同样调用 `InternalExecuteMod` 修改Attribute.

**Invoke GameplayCue events**<br>
如果上面的`Modifiers`或`Executions` 不是空数组，则触发GameplayCue.
...
输入系统非常重要，这是人机合一的重要途径， GAS系统如何与人类交互？

这一部分归于AbilityTask章节，由相关Task引入.

`WaitInputPressed/Released` <br>
`WaitConfirmCancel`
...
##### Callbacks / Notifies
在`AbilitySystemComponent.h` 搜索 `Callbacks / Notifies` 就可以跳转到对应的位置，<br>
有很多可以绑定的代理.
...
```
每个 UCLASS 都保留一个称作 类默认对象 的对象，简称CDO.CDO 本质上是一个默认"模板"对象，由类构建函数生成，之后就不再修改.
```
...
void UGameplayAbility::EndAbility(const FGameplayAbilitySpecHandle Handle, /*...*/)
{
    ...
    // Empty the array but don't resize memory, since this object is probably going to be destroyed very soon anyways.
	ActiveTasks.Reset();
}
```
...
##### WaitInputPressed/Released

在技能需要 按住再释放 或 再次按下时触发额外效果 时，<br>
`WaitInputPress` 可用来等待第二次按下（或同一次按下，根据 `bTestInitialState` 参数）
...
上面`AbilitySpecInputPressed`的注释里说不支持 `bReplicateInputDirectly`，追溯一下就可以发现这和旧的输入绑定有关，<br>
...
{
    /*...*/
    if (!ASC->CallReplicatedEventDelegateIfSet(EAbilityGenericReplicatedEvent::InputPressed, ...))
    {
...
`CallReplicatedEventDelegateIfSet` 检查服务器是否已经提前从客户端收到了 `InputPressed` 的复制事件，<br>
例如，由于网络顺序，RPC 先于任务的 `Activate` 到达.<br>
如果已经存在，它会立即触发委托，直接调用 `OnPressCallback`.

如果还没有收到，则调用 `SetWaitingOnRemotePlayerData`，表示需要等待远程数据.<br>
这会让 ASC 知道该能力正在等待来自客户端的异步确认.

`UAbilityTask_WaitInputPress::OnPressCallback` :<br>
当按下按键时，这个函数就执行了，它已经做好了服务器和客户端的支持，<br>

在客户端上会创建预测Key，通过`ServerSetReplicatedEvent`告诉服务器按下了按键，<br>

在服务器上就不创建预测Key了，调用 `ConsumeGenericReplicatedEvent` 清除 `AbilityTargetDataMap` 中存储的 `InputPressed` 事件标记

最后 服务器和客户端都会广播 `OnPress`，从而在蓝图中执行后续任务.

要注意的是，这里已经有了预测窗口，所以在后续执行的任务是安全的，至少在GAS上有了回滚的保障.

---

##### WaitConfirmCancel

确认/取消 技能是十分重要的功能，在 `Paragon` 里面有很多这样的技能，<br>
通过这个Task就可以方便的实现相关的按键判定.

直接调用函数就好了 : 写在蓝图里是为了避免增强输入中 不同Context之间优先级的问题.

![](Meta/img2/Paragon_Confirm.png)

客户端的`Activate`函数中，如果Actor是本地控制的，就绑定`GenericLocalConfirmCallbacks`委托，<br>
当委托触发时，调用 `ServerSetReplicatedEvent` ，告诉服务器确认事件，接着就广播`OnConfirm` ，执行蓝图里的逻辑.<br>

服务器在`Activate`函数中监听了`GenericConfirm`事件，当客户端告诉服务器确认事件时，服务器就可以做出反应，<br>
不过服务器是直接执行`OnConfirmCallback`.

---

##### WaitConfirm

这个并不是等待按键确认，而是在等待服务器通过判定 确认可以执行技能.

客户端预测激活技能，`ActivationInfo.ActivationMode = Predicting`.<br>
服务器验证成功，调用 `ClientActivateAbilitySucceed` RPC.<br>
客户端收到该 RPC 后，<br>
在 `ClientActivateAbilitySucceed_Implementation` 中会调用 `Ability->ConfirmActivateSucceed()`,<br>
广播 `OnConfirmDelegate` 委托.<br>

客户端:<br>
技能预测激活：以 `LocalPredicted` 策略激活技能，`ActivationInfo.ActivationMode = Predicting`.<br>

执行到 `WaitConfirm` 节点：<br>
`Activate()` 检测到模式为 `Predicting`，于是向 `Ability->OnConfirmDelegate` 注册回调.<br>
等待服务器的`OnConfirm`.<br>

等待服务器响应：<br>
如果服务器确认成功 → 客户端收到 `ClientActivateAbilitySucceed` RPC <br>
→ 调用 `ConfirmActivateSucceed()` → 广播 `OnConfirmDelegate` <br>
→ `WaitConfirm` 广播 `OnConfirm`，执行后续不可预测的逻辑.<br>
如果服务器拒绝 → 客户端收到 `ClientActivateAbilityFailed` <br>
→ GA被强制结束，`WaitConfirm`随GA销毁.<br>
...
{
	OnApplyGameplayEffectCallbackDelegateHandle = GetASC()->OnGameplayEffectAppliedDelegateToSelf
    .AddUObject(this, &UAbilityTask_WaitGameplayEffectApplied::OnApplyGameplayEffectCallback);

	if (ListenForPeriodicEffects)
	{
		OnPeriodicGameplayEffectExecuteCallbackDelegateHandle = GetASC()->OnPeriodicGameplayEffectExecuteDelegateOnSelf
        .AddUObject(this, &UAbilityTask_WaitGameplayEffectApplied::OnApplyGameplayEffectCallback);
	}
}
```
关键在于 `OnGameplayEffectAppliedDelegateToSelf` `OnPeriodicGameplayEffectExecuteDelegateOnSelf` 这两个委托，<br>
...
`OnGameplayEffectAppliedDelegateToSelf` 和 `ToTarget` 的触发时机：
...
FActiveGameplayEffectHandle UAbilitySystemComponent::ApplyGameplayEffectSpecToTarget(/*..*/)
{
...
`OnGameplayEffectAppliedToSelf` `OnGameplayEffectAppliedToTarget` 就是触发委托的地方.

委托的注释说只在服务器上触发，通过代码来看 这个描述并不准确，<br>
在应用GE时，`HasNetworkAuthorityToApplyGameplayEffect` 会检测 NetworkAuthority 以及预测Key的有效性，<br>
如果没有权限或者预测Key无效 就不执行下文了.

在客户端上 预测Key有效，通过验证，进而执行后续的代码 应用GE.

所以 服务器和客户端都会应用GE，在客户端上面也会触发委托.
...
免疫阻挡事件的判定必须依赖服务器，客户端无法可靠预测.<br>
注释中的 `Clients could mispredict` ：如果允许客户端参与，就会因状态不一致而导致错误.
...
主要实现方法是绑定GE的 `EventSet.OnEffectRemoved`.<br>
...
-> FActiveGameplayEffectsContainer::InternalOnActiveGameplayEffectRemoved
```

路线2 : 由蓝图可调用的Remove函数触发
```cpp
...
路线3 : 在应用GE时 设置的定时器，定时器时长是GE的持续时间，<br>
到期后 移除GE.
```cpp
...
这个定时器的Handle是 `FActiveGameplayEffect::DurationHandle` ,<br>
`CheckDuration` 会重新检查确认GE的持续时间，避免因为Stack刷新策略而错误判定GE要移除，<br>
如果GE的持续时间被刷新了，就重新设置一个定时器 绑定在`DurationHandle`上，与应用GE时的绑定操作一致.

路线4 : 通过ASC根据匹配条件 移除GE
```cpp
UAbilitySystemComponent::RemoveActiveEffects
...
`RemoveActiveGameplayEffectBySourceEffect` 根据GE的CDO判断是否要移除.<br>
`RemoveActiveEffects`是更通用的，可自定义匹配条件.
...
需要与 `SendGameplayEvent` 配合使用，具有预测功能.
...
`UAbilitySystemComponent::HandleGameplayEvent` :<br>
查询 `GameplayEventTriggeredAbilities` ，如果有通过EventTag激活的技能，那么就激活这些技能.

查询 `GenericGameplayEventCallbacks` 与EventTag对应的委托，精准匹配.<br>
查询 `GameplayEventTagContainerDelegates` 与EventTag对应的委托，使用 `MatchesAny` 模糊匹配，<br>

`MatchesAny`:<br>
"A.1".MatchesAny({"A","B"}) will return True.<br>
"A".MatchesAny({"A.1","B"}) will return False.<br>

`AbilityTask_WaitGameplayEvent` 提供了精准匹配与模糊匹配的选择，默认是精准匹配，`OnlyMatchExact = true`.

---

##### WaitGameplayTagAdded/Removed

监听ASC的Tag情况， 在GE中使用 `TargetTagsGameplayEfffectComponent` 添加或移除Tag.
...
`GameplayTagCallback` 由 `WaitGameplayTagAdded` 和 `WaitGameplayTagRemoved` 实现<br>
通过`NewCount` 判断Add或者Remove.

---

##### WaitOverlap

绑定组件的 `OnComponentHit` 事件，默认是胶囊体组件.

事件触发后，构造一个 `TargetData` 传给蓝图.

如果要自定义绑定的组件类型，只能创建一个新的Task，因为 `GetComponent()` 函数不是virtual， 不能重写.

---

##### WaitAttributeChange

指的是 `AbilityTask_WaitAttributeChange` . 

有 `WaitForAttributeChange` 和 `WaitForAttributeChangeWithComparison` 两种类型，<br>
`WaitForAttributeChange` 只是监听属性变化，只要变化了 就触发蓝图的委托.<br>
`WaitForAttributeChangeWithComparison` 可以对比属性是否大于、等于、小于某个值，对比成功才触发蓝图的委托.

`WithTag`：要求引起属性变化的 GE 的来源标签必须包含该标签.<br>
如果 `WithTag` 有效但来源标签中没有它，则忽略此次变化.

`WithoutTag`：要求引起属性变化的 GE 的来源标签不能包含该标签.<br>
如果 `WithoutTag` 有效且来源标签中有它，则忽略此次变化.

希望技能等待生命值降低，但仅当降低效果来自“毒素”标签的 GE 时，可以设置：<br>
Attribute = 生命值<br>
WithTag = “Damage.Type.Poison”<br>

如果希望等待生命值增加，但排除来自“治疗”标签的 GE，可以设置：<br>
Attribute = 生命值<br>
WithoutTag = “Effect.Type.Healing”<br>
...
监听ASC的 `AbilityCommittedCallbacks` 委托.
...
触发途径 : `CommitAbility` `K2_CommitAbility` `K2_CommitAbilityCooldown` `K2_CommitAbilityCost`

---

##### WaitAbilityActivate

监听ASC的 `AbilityActivatedCallbacks` 委托.
...
调用 `EndAbilityState` 结束技能时 输出一个FName，<br>

`StartAbilityState`可以根据FName的内容 提供不同的执行策略.

这个Task的实现方式是监听GA的 `OnGameplayAbilityStateEnded` .

触发途径 : GA蓝图调用 `EndAbilityState` 即可.
```cpp
...
判断 `EndAbilityState` 传来的Name是不是想要的那个，如果是的话 结束Task.<br>
在 `OnDestroy` 里面广播蓝图的委托.
```cpp
...
此外 还监听了GA的`OnGameplayAbilityCancelled`事件，例如 A技能取消了B技能，`OnInterruptState` 事件就会执行.<br>

---

##### PlayMontageAndWait

在多人游戏时，有可能出现蒙太奇动画不会播放的问题，只会在本地客户端播放.<br>
解决方法是 将骨骼网格体的 `VisibilityBasedAnimTickOption` 设置为 `AlwaysTickPose and RefreshBones`.

GAS的蒙太奇已经做好了网络同步和预测相关的操作，直接用就可以了.

播放蒙太奇动画需要用到骨骼的动画蓝图Instance，
```cpp
const FGameplayAbilityActorInfo* ActorInfo = Ability->GetCurrentActorInfo();
UAnimInstance* AnimInstance = ActorInfo->GetAnimInstance();
```
而ActorInfo的骨骼网格体只有在 `InitAbilityActorInfo` 时会设置一次，所以不会实时更新，<br>
如果骨骼网格体被摧毁了 并且没有再次调用 `InitAbilityActorInfo` ， 那么之后就无法获取骨骼网格体了.

在播放蒙太奇动画时，GA也会保存当前所播放的蒙太奇 ：
```cpp
float UAbilitySystemComponent::PlayMontage(UGameplayAbility* InAnimatingAbility/*...*/)
{
...
`LocalAnimMontageInfo` :  客户端/服务器 存储本地实际播放的蒙太奇状态.<br>
`RepAnimMontageInfo` : 服务器 → 所有客户端 用于同步权威蒙太奇信息.
...
```
`OnRep_ReplicatedAnimMontage` 在接到同步后，播放对应的蒙太奇动画.

`UAbilitySystemComponent::PlayMontage` 还注册了预测回滚，
```cpp
...
预测Key 在激活GA时就会生成，这里获取的也是当前GA的预测Key，当GA被服务器拒绝时 可以做出对应的操作 取消蒙太奇的播放.
```cpp
...
`CurrentMontageStop` 会调用 `AnimMontage_UpdateReplicatedData` ，查询蒙太奇的播放状态，<br>
如果停止播放了，就把 `IsStopped` 设为true，等它同步到其他客户端的时候，那些客户端根据 `IsStopped` 去停止蒙太奇播放.
...
*
*		- 原本可以使用 UObject 来提供多态性和在蓝图中通过引用传递，但在复制方面我们仍然会遇到问题
*
*		- 按值复制
*		- 在蓝图中通过引用传递
*		- 目标数据结构的多态性
...
逐条解释:<br>
**避免在蓝图中复制完整的目标数据结构**<br>
如果直接使用 `FGameplayAbilityTargetData` 这样的结构体作为函数参数或返回值，每次传递都会发生深拷贝，整个数据都会被复制.<br>
`TargetDataHandle` 作为一个轻量级的 Handle，蓝图中传递的是这个`Handle`本身，而非实际的 `TargetData`.

`Handle` 存储了`UniqueId` 和 一个共享指针保存的 `FGameplayAbilityTargetData`，其构造函数实现了右值Move，<br>
在发生拷贝构造时，只是复制`UniqueId`、增加 `FGameplayAbilityTargetData` 的引用计数.<br>
因此，无论蓝图中如何传递这个 `Handle`，底层的目标数据始终只有一份.

---

**允许我们利用目标数据结构的多态性**

实际的 `TargetData` 可能是多种不同类型，自带的就有 `_SingleTargetHit` `_ActorArray` `_LocationInfo`.<br>

当添加一个新的 `TargetData` 时，通过 `new FLyraGameplayAbilityTargetData_SingleTargetHit(...)` 创建子类对象，<br>
然后调用 `Handle.Add(ptr)`，使用共享指针保存的 `Data` 便自然获得了多态能力.<br>

---

**允许我们实现 NetSerialize 并在客户端/服务器之间按值复制**

这一段在分析 `FGameplayAbilityTargetDataHandle::NetSerialize` 函数.

网络复制时，需要将 `Handle` 所包含的所有`TargetData`打包成字节流，发送到远端，然后完整地重建出相同的 `Handle` 内容.<br>
这需要支持动态类型序列化，并且整个过程对上层表现为 `按值复制` , 即接收方获得一个内容完全相同的独立 `Handle`.

`FGameplayAbilityTargetDataHandle` 实现了 `NetSerialize` 函数，并标记 `WithNetSerializer = true`.

序列化流程：

- 写入/读取 `UniqueId`.
- 写入/读取元素个数 `DataNum`（uint8，最多 255 个）.

对每个元素：
- 通过 `GetScriptStruct()` 获取实际派生类的 `UScriptStruct*` 类型信息.
- 使用 `UAbilitySystemGlobals::Get().TargetDataStructCache.NetSerialize()` 将类型信息序列化为1 字节索引（而非完整 UObject 路径），见 `FNetSerializeScriptStructCache`.
- 如果是加载端，根据类型索引动态分配正确派生类对象的内存，并调用其 `InitializeStruct`.
- 最后调用该结构体的 `NetSerialize`，要求每个派生类必须实现自己的 `NetSerialize` 来读写具体成员数据.

接收端反序列化后，得到全新的 `TSharedPtr` 数组，内容与发送端完全一致.这就是 `按值复制` 的语义.

把 结构体类型 序列化为1字节索引是如何做到的 ? <br>
...
```
整体就是在 `ScriptStructs` 里面去找结构体的索引，<br>
`ScriptStructs` 在 `UAbilitySystemGlobals` 初始化时 收集带有反射的结构体，根据名字做一个排序 保证每次启动都是一样的顺序，<br>
当顺序确定一致后，服务器只要发送一个索引，客户端之间就能够通过索引 在容器中找到匹配的结构体.<br>
最终得到多态环境下的 `TargetData` 的具体类型.

这个功能要求子类必须实现这个函数，以便在序列化时 使用正确的结构体类型:<br>
...
```
`InitializeStruct`内部会调用 `FMemory::Memzero`，<br>
除此之外，还会初始化反射结构体的内存 :
```cpp
...
当反射系统认为某个属性应该存在，但 C++ 结构体中没有对应的内存时 例如在热重载或蓝图结构体中 由反射系统额外分配空间，<br>
通过 `PropertyLink` 遍历这些属性，调用 `InitializeValue_InContainer` 来初始化它们.<br>
这种情况通常只发生在动态生成的结构体或蓝图中定义的结构体上，对于常规的 C++ USTRUCT，`PropertiesSize` 应该等于 `InitializedSize`.

判断是否有实现`NetSerialize`函数 :
```cpp
...
- `UObject` 的复制是基于对象引用的，需要提前在网络系统中注册、复制 CDO 或动态生成的 `Actor`，管理复杂.
- `TargetData`往往是临时、短暂的对象，若用 `UObject`，则需要频繁创建/销毁并手动管理网络实例化，开销大且容易出错.
- 蓝图传递 `UObject` 指针时，需要保证对象在整个延迟过程中不被GC.
