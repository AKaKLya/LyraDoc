...
`DeclVal<T>()` 是一个只有声明、没有定义的函数模板，用于在编译期的不求值上下文中“产生”一个类型为 `T&&` 的值.它是对 C++ 标准库 `std::declval` 的等效实现.
...
AllOf — 是否全部满足条件
```cpp
TArray<int32> Numbers = {2, 4, 6, 8};
bool bAllEven = Algo::AllOf(Numbers, [](int32 N) { return N % 2 == 0; }); // true

// 无谓词版本：元素自身是否为 truthy
TArray<bool> Flags = {true, true, true};
bool bAllTrue = Algo::AllOf(Flags); // true
```

AnyOf — 是否存在满足条件的元素
```cpp
TArray<int32> Numbers = {1, 2, 3};
bool bHasEven = Algo::AnyOf(Numbers, [](int32 N) { return N % 2 == 0; }); // true
bool bAnyTrue = Algo::AnyOf(TArray<bool>{false, false, true});            // true
```

NoneOf — 是否全部不满足条件
```cpp
TArray<int32> Numbers = {1, 3, 5};
bool bNoneEven = Algo::NoneOf(Numbers, [](int32 N) { return N % 2 == 0; }); // true

// 不带投影：元素自身是否为 truthy
TArray<bool> Flags = {false, false, false};
bool bNoneTrue = Algo::NoneOf(Flags); // true
```

Count / CountIf / CountBy — 计数
```cpp
TArray<int32> Numbers = {1, 2, 2, 3, 2, 4};

// 统计值等于2的个数
int32 CountTwo = Algo::Count(Numbers, 2); // 3

// 统计偶数个数
int32 CountEven = Algo::CountIf(Numbers, [](int32 N) { return N % 2 == 0; }); // 4

// 统计投影后等于“even”的个数
auto Proj = [](int32 N) { return N % 2 == 0 ? "even" : "odd"; };
int32 CountEvenStr = Algo::CountBy(Numbers, FString("even"), Proj); // 4
```

Includes — 判断已排序容器的子序列关系
```cpp
TArray<int32> SortedMain = {1, 2, 3, 4, 5, 6};
TArray<int32> SortedSub = {2, 4, 6};

bool bIncludes = Algo::Includes(SortedMain, SortedSub); // true
// 支持自定义谓词和投影
bool bIncludesBy = Algo::IncludesBy(SortedMain, SortedSub, FIdentityFunctor(), TLess<>());
```
...
Sort — 不稳定排序
```cpp
TArray<int32> Numbers = {3, 1, 4, 1, 5, 9};
Algo::Sort(Numbers); // 升序 {1,1,3,4,5,9}
Algo::Sort(Numbers, TGreater<>()); // 降序
Algo::SortBy(Numbers, [](int32 N) { return -N; }); // 按负值排序（降序）
```

StableSort — 稳定排序
```cpp
struct FItem { int32 Key; int32 OriginalOrder; };
TArray<FItem> Items = { {2,0}, {1,1}, {2,2} };
Algo::StableSort(Items, [](const FItem& A, const FItem& B) { return A.Key < B.Key; });
// 结果：{1,1}, {2,0}, {2,2} （Key=2的两个保持原相对顺序）
```

HeapSort — 堆排序
```cpp
TArray<int32> Numbers = {5, 2, 9, 1, 5};
Algo::HeapSort(Numbers); // 排序后 {1,2,5,5,9}
```

Heapify — 构建堆
```cpp
TArray<int32> Numbers = {3, 1, 4, 1, 5};
Algo::Heapify(Numbers); // 转换为最小堆（根为最小值）
```
...
TArray<int32> Dest;
Algo::Copy(Source, Dest); // Dest == {1,2,3,4,5}

TArray<int32> EvenDest;
Algo::CopyIf(Source, EvenDest, [](int32 N) { return N % 2 == 0; }); // {2,4}
```

Transform / TransformIf — 变换并存储
```cpp
TArray<int32> Input = {1, 2, 3};
TArray<FString> Output;
Algo::Transform(Input, Output, [](int32 N) { return FString::FromInt(N); });
// Output == {"1","2","3"}

// 多路输出（使用 TieTupleAdd）
TArray<FString> Strings;
TArray<int32> Squares;
Algo::Transform(Input, Algo::TieTupleAdd(Strings, Squares),
    [](int32 N) { return MakeTuple(FString::FromInt(N), N*N); });
// Strings = {"1","2","3"}; Squares = {1,4,9}
```

Reverse — 反转
```cpp
TArray<int32> Numbers = {1, 2, 3, 4};
Algo::Reverse(Numbers); // {4,3,2,1}
```
Rotate — 旋转
```cpp
TArray<int32> Numbers = {1, 2, 3, 4, 5};
Algo::Rotate(Numbers, 2); // 前2个移到末尾 -> {3,4,5,1,2}
```

Replace / ReplaceIf — 替换
```cpp
TArray<int32> Numbers = {1, 2, 3, 2, 4};
Algo::Replace(Numbers, 2, 99); // {1,99,3,99,4}
Algo::ReplaceIf(Numbers, [](int32 N) { return N > 50; }, 0); // {1,0,3,0,4}
```

RemoveIf / StableRemoveIf — 移除元素（返回新逻辑大小）
```cpp
TArray<int32> Numbers = {1, 2, 3, 4, 5};
int32 NewSize = Algo::RemoveIf(Numbers, [](int32 N) { return N % 2 == 0; });
Numbers.SetNum(NewSize); // Numbers 变为 {1,3,5}
```

Unique — 移除连续重复元素
```cpp
TArray<int32> Numbers = {1, 1, 2, 2, 3, 1, 1};
int32 NewSize = Algo::Unique(Numbers); // 返回新逻辑大小
Numbers.SetNum(NewSize); // {1,2,3,1} （注意最后两个1不连续，保留一个）
```

Partition — 分割（满足谓词放前面）
```cpp
TArray<int32> Numbers = {1, 2, 3, 4, 5};
int32 Pivot = Algo::Partition(Numbers, [](int32 N) { return N % 2 == 0; });
// 前 pivot 个元素都是偶数，但不保证内部顺序；pivot 值为偶数个数（2）
```

RandomShuffle — 随机打乱
```cpp
TArray<int32> Numbers = {1, 2, 3, 4, 5};
Algo::RandomShuffle(Numbers); // 随机排列
```

---
**聚合与累积**

Accumulate — 求和（或自定义累积）
```cpp
TArray<int32> Numbers = {1, 2, 3, 4};
int32 Sum = Algo::Accumulate(Numbers, 0); // 10
int32 Product = Algo::Accumulate(Numbers, 1, TMultiplies<>()); // 24
```

TransformAccumulate — 先映射再累积
```cpp
TArray<int32> Numbers = {1, 2, 3};
int32 SumOfSquares = Algo::TransformAccumulate(Numbers, [](int32 N) { return N*N; }, 0);
// 1+4+9 = 14
```
...
Compare — 比较两个容器是否相等
```cpp
TArray<int32> A = {1, 2, 3}, B = {1, 2, 3};
bool bEqual = Algo::Compare(A, B); // true

// 使用谓词（如近似比较）
TArray<float> X = {0.1f, 0.2f}, Y = {0.101f, 0.199f};
bool bApproxEqual = Algo::Compare(X, Y, [](float a, float b) { return FMath::IsNearlyEqual(a,b); });
```

LevenshteinDistance — 编辑距离
```cpp
FString A = TEXT("kitten");
FString B = TEXT("sitting");
int32 Distance = Algo::LevenshteinDistance(A, B); // 3
```
...
```cpp
TArray<int32> Numbers = {3, 1, 4, 1, 5};
int32* MinPtr = Algo::MinElement(Numbers); // 指向第一个1
int32* MaxPtr = Algo::MaxElement(Numbers); // 指向5

// 按投影：找到年龄最小的人
struct FPerson { FString Name; int32 Age; };
TArray<FPerson> People = {{TEXT("Alice"), 30}, {TEXT("Bob"), 25}};
FPerson* Youngest = Algo::MinElementBy(People, &FPerson::Age); // 指向Bob
```
...
```cpp
TArray<int32> Numbers = {1, 2, 3, 4, 5};
Algo::ForEach(Numbers, [](int32& N) { N *= 2; }); // 每个元素翻倍
Algo::ForEachIf(Numbers, [](int32 N) { return N > 5; }, [](int32& N) { N = 0; });
// 大于5的置0
```
...
如果TArray需要扩容，`realloc`会将原地址的内容复制到新地址，释放原地址的内存.

[点击查看具体代码](https://github.com/microsoft/mimalloc/blob/main/src/alloc.c#L284)

```cpp
/*将内存重新分配到 newsize 字节.
@param p 指向之前分配的内存块的指针（或 NULL）.
@param newsize 新的所需大小（字节数）.
@returns 返回指向重新分配的内存的指针，
大小为 newsize 字节；如果内存不足则返回  NULL.
如果返回  NULL，则指针  p 不会被释放.
否则，原始指针要么被释放，要么作为重新分配的结果返回（当它能原地适配新大小时）.
如果指针 p 是  NULL，则行为如同 mi_malloc(\a newsize).
如果 newsize 大于为 p 分配的原始 size，则 size 之后的字节未初始化.
@see mi_reallocf()
*/
...
{
	// 检查是否可以原地扩展（条件：newsize <= size 且 newsize >= size/2）
	if mi_unlikely(newsize <= size && newsize >= (size / 2) && newsize > 0) 
	{
	    return p;  // 原地扩展，地址不变，元素未移动
	}
...
`FName` 存储方式：一个指向唯一字符串表的索引 + 一个实例编号.<br>
不区分大小写：比较两个 `FName` 时，"Actor"、"actor"、"ACTOR" 被视为相等.这是因为比较的是它们归一化后的索引，而不是原始字符.<br>
保留大小写：创建 `FName` 时传入的大小写形式会被存储（例如 `PlayerController` 显示时就是该形式），这样在调试、序列化、UI 显示时仍能看到原始大小写，具有可读性.

在此系统中，特定字符串即使会被重复使用，在数据表中也只存储一次.<br>
`FNames` 不区分大小写.它们为不可变，无法被操作.`FNames` 的存储系统和静态特性决定了通过键进行 `FNames` 的查找和访问速度较快. <br>
`FName` 子系统的另一个功能是使用散列表为 `FName` 转换提供快速字符串.<br>
`FNames` 不区分大小写，作为索引组合存储在唯一字符串和实例编号的表格中.<br>
...
{}
```
`MakeDetectNumber` 返回一个FName，委托给了FName(const FName&)来构造，
...
```cpp
FName("Weapon_0"), FName("Weapon_1"), ..., FName("Weapon_999")
```

没有 `ParseNumber` 时：每个 `FName` 都会把完整的字符串（如 `Weapon_0`、`Weapon_1` ）作为一个独立的条目存入全局名称表.

`ParseNumber`：<br>
所有 1000 个 `FName` 共享同一个基名字符串 `Weapon` 只存一份.<br>
每个 `FName` 只存储一个整数（数字部分，如 0、1……）.<br>
比较时先比较基名索引（整数，很快），再比较数字，效率极高.<br>


```cpp
/** 指向名称数组的索引（用于查找字符串/数字对中 用于比较 的字符串部分） */
FNameEntryId ComparisonIndex;

/** 字符串/数字对中的数字部分（内部存储为实际值加1，这样零初始化的内存将表示默认的“无数字”情况） */
uint32 Number;

#if WITH_EDITORONLY_DATA
/** 指向名称数组的索引（用于查找字符串/数字对中 用于显示 的字符串部分） */
FNameEntryId DisplayIndex;
#endif
```

`ComparisonIndex` 是一个整数索引，通过它可以快速定位到全局池中的 `FNameEntry`（存储实际字符串 "Weapon"）.<br>
`Number` 存储数字部分（外部显示 `_123` 时，这里存 `124`，因为内部用 `0` 表示无数字）.<br>
因此 `FName` 对象本身不存储完整字符串，只是两个整数（通常 8 字节），非常轻量.<br>

`ParseNumber` 具体做了什么事？<br>
输入一个字符串（例如 `Weapon_123`）和它的长度（Len = 10），<br>
从末尾向前扫描，找到连续的数字字符 `123`，得到 `Digits = 3`.<br>
检查数字前一个字符是否为 `_`，且数字长度合理（≤10，无前导零等）.<br>
如果合法，将数字部分转换为整数 `123`.<br>
修改 `Len` 为基名长度 `6`（去掉 `_123` 后的 `Weapon` 长度）.<br>
返回 内部数字 `124`（外部数字 `123` + 1）.<br>
这样 `FNameHelper::MakeDetectNumber` 就可以用 `Weapon`（长度 6）去名称池中查找/添加基名，并单独保存数字 `124` 到 `FName` 的 `Number` 中.<br>
...
```
`FNameDisplayValue` 在构造时 通过字符串和字符串的长度 计算出一个哈希值 塞进Pool里面，<br>
后续再次构造这个FName时，只要值相同，就可以从哈希表里面快速找出来.
...
`SpawnActor` `NewObject` 如何从CDO里面 把属性复制到新创建的对象中 ?<br>

上一节 `SpawnActor` 的图中，有一段关于 `FObjectInitializer` 的部分.

在 `NewObject` 创建对象时 通过 `FObjectInitializer` 的构造和析构 完成对象的创建 包括属性复制.

析构时调用 `PostConstructInit` : 
...
void FObjectInitializer::PostConstructInit()
{
    ...
    // 决定是否需要实例化子对象
    bool bNeedInstancing = !bIsCDO && ObjectArchetype != nullptr;
    bool bNeedSubobjectInstancing = !bIsCDO && ComponentInits.SubobjectInits.Num() > 0;

    // 初始化普通属性（从 CDO 复制）
    InitProperties(Obj, Class, ObjectArchetype ? ObjectArchetype : Class->GetDefaultObject(), false);
...
```
`AMyActor` 的构造函数是在`NewObject`里面调用的，`NewObject`又有一个`FObjectInitializer`.

`CreateDefaultSubobject` 这个函数实际上就是在调用 `FObjectInitializer` .
```cpp
...
```
梳理一下，`NewObject` 创建了`FObjectInitializer`.<br>
`FObjectInitializer` 在构造时，就把自己添加到`FUObjectThreadContext` 中，<br>
之后 调用 `AMyActor::AMyActor`，通过全局的`FUObjectThreadContext` 获取`FObjectInitializer`.

`CreateDefaultSubobject` 调用 `StaticConstructObject_Internal` 创建组件，其中又有一个新的`FObjectInitializer`来初始化组件.<br>
当创建组件的`FObjectInitializer`析构时，从组件CDO构造组件.

```cpp
UObject* FObjectInitializer::CreateDefaultSubobject(/*...*/)
{
...
当组件构造完毕后 就可以调用这个组件的函数了.


---
`FObjectInitializer` 里面也有公开的函数，可以使用带参数版本的构造函数 直接调用`ObjectInitializer`的函数.
```cpp
...
`MetaData` 是指 `UFUNCTION`  `UPROPERTY` 等标记参数中的 `meta`.<br>

例如，要限制 `GameplayTag` 的选择范围，在编辑器中只能选择某个Tag下的子Tag，就可以通过MetaData来实现:
```cpp
UPROPERTY(EditDefaultsOnly,BlueprintReadOnly,meta = (categories = "MyAbility"))
...
`MetaData` 的原理:
```cpp
UPROPERTY(EditDefaultsOnly,BlueprintReadOnly,meta = (AssetFilterPrefix = "T_Ability_"))
UTexture2D* AbilityIcon;
```
UHT会在 `.gen.cpp` 中生成以下代码 ：
```cpp
#if WITH_METADATA
	const UECodeGen_Private::FMetaDataPairParam Z_Construct_UScriptStruct_FAbilityIcon_Statics::NewProp_AbilityIcon_MetaData[] = {
		{ "AssetFilterPrefix", "T_Ability_" },
	};
#endif
```

`FMetaDataPairParam` 是一个结构体：
...
PropertyType* NewFProperty(FFieldVariant Outer, const FPropertyParamsBase& PropBase)
{
	...
	for (/*...*/)
	{
		NewProp->SetMetaData(UTF8_TO_TCHAR(MetaDataData->NameUTF8), 
		UTF8_TO_TCHAR(MetaDataData->ValueUTF8));
	}
#endif
}
```

既然最终还是在操作 `FProperty`，那么利用反射系统 不是也可以获得 `FProperty` 进行操作 ?

---

```cpp
TMap<FName,FString> MetaData;
MetaData.Add("categories","MyAbility");
...
```cpp
for (TFieldIterator<FProperty> It(UGameplayAbility::StaticClass()); It; ++It)
{
...
`MetaData` 是可以随意添加信息的，可以自己定义一个字段实现一些编辑器功能.<br>
例如: 在选择贴图时 只能选择固定前缀的贴图.

只能选择 `T_` 开头的贴图资产 :

![](Meta/img2/MetaData_eg.png)

只需要这样定义 :

```cpp
UPROPERTY(EditDefaultsOnly,meta = (AssetFilterPrefix = "T_"))
UTexture2D* Tex;
```

具体实现代码在 `PropertyExtend` 这个插件中 的这个类里 :

```cpp
class FPropertyExtendEditor : public IPropertyTypeCustomization
```
...
这个东西究竟有什么用？ 大概是可以自由控制资产的加载和卸载.

资产是重要的数据，硬盘上的 `.asset` 由引擎自动加载， 但有时候需要更精细的控制加载的资产.

资产类型分为 `主资产` `次资产`，`主资产` 拥有ID，可以利用ID直接对其进行操作，例如加载、卸载.<br>
`次资产` 只能在主资产引用时加载.

普通的 `UDataAsset` 仅是一组静态数据的容器，而 `UPrimaryDataAsset` 拥有一个`PrimaryAssetId`<br>
只要有了这个 ID，`AssetManager` 就可以在不知道资产具体文件路径的情况下，通过 ID 找到并加载该资产.

例如，定义了很多装备物品 又不想手动一个个加入列表中，就可以用 `GetPrimaryAssetIdList` 实现自动加载.

`AssetManager` 底层使用的是 `AssetRegistry` 和 `StreamableManager`.

---
**主资产操作**<br>

`GetPrimaryAssetIdList`：获取指定类型的所有资产 ID.<br>
`GetPrimaryAssetData`：通过 ID 获取其 FAssetData（包含标签、路径等但不加载资产）.<br>
`GetPrimaryAssetTypeInfo`：获取某种资产类型的配置信息（如扫描路径、是否在包中等）.<br>

初始化背包界面.<br>
游戏启动时，需要展示所有道具，调用 `GetPrimaryAssetIdList` 获取所有道具 ID，<br>
然后调用 `GetPrimaryAssetData` 获取它们的 “显示名称” 和 “稀有度” 标签，而不需要真正加载几百个道具的物理模型.

---

**动态异步加载**

`LoadPrimaryAssets` / `LoadPrimaryAsset`：异步加载一个或一组主资产.<br>
`LoadPrimaryAssetsWithType`：一键异步加载某种类型下的所有资产.<br>
`UnloadPrimaryAsset`：释放已加载的资产，手动管理内存.<br>

玩家进入关卡.<br>
当玩家走进“沙漠地区”时，使用 `LoadPrimaryAssets` 加载该区域特有的敌人和植被资产.<br>
当玩家离开并进入“雪地”时，使用 `UnloadPrimaryAsset` 卸载沙漠资产.

---
**资产束 Asset Bundles**

资产束是 `UPrimaryDataAsset` 内部的分组（如 "Visual", "Sound"），允许部分加载资产.

`LoadPrimaryAssetSubset`：只加载资产中某个 Bundle 标记的部分.<br>
`ChangeBundleStateForPrimaryAssets`：动态切换一组资产的加载束状态.<br>

画质分级加载.<br>
一个角色资产包含 "Mesh" 束和 "Sound" 束,在某些不需要播放声音的预览界面，只想加载模型.<br>
通过 `LoadPrimaryAssetSubset` 并指定 Bundle 为 "Mesh"，可以避免加载不必要的音效文件.<br>
...
```
`AssetManager` 在 `Engine` 中创建，之后调用两个 `InitialLoading`.
...
Timer 的 Tick 时机 : Actor::Tick 在 Timer之前.
```cpp
void UWorld::Tick( ELevelTick TickType, float DeltaSeconds )
{
    if (bDoingActorTicks)
    {
        // 1. 先执行所有 Actor 的 Tick 组
...
`Windows` 使用消息传递模型,操作系统通过向应用程序窗口传递消息来与其通信.<br>
消息只是指定特定事件的数值代码，例如，如果用户按下鼠标左键，窗口将收到一条消息，消息代码如下:

```cpp
#define WM_LBUTTONDOWN    0x0201
```

某些消息具有与其关联的数据. 例如 `WM_LBUTTONDOWN` 消息包括鼠标光标的 x 坐标和 y 坐标.<br>
若要向窗口传递消息，操作系统将调用为该窗口注册的窗口过程.
...
```
`lpfnWndProc`:指向窗口过程的指针，它需要绑定一个函数. <br>
...
```
`hwnd` 是窗口的句柄.<br>
`uMsg` 是消息代码；例如，`WM_SIZE` 消息指示窗口已调整大小.<br>
`wParam` 和 `lParam` 包含与消息相关的其他数据， 具体含义依赖于消息代码.<br>

`LRESULT` 是程序返回到 `Windows` 的整数值，它包含程序对特定消息的响应，此值的含义取决于消息代码.

如果不在窗口过程中处理特定消息，请将消息参数直接传递到 `DefWindowProc` 函数. <br>
此函数对消息执行默认操作，该操作因消息类型而异.

示例 : 获取鼠标位置 <br>
`WM_LBUTTONDOWN` 消息包括鼠标光标的 x 坐标和 y 坐标,<br>
`lParam`<br>
低序单词指定光标的 x 坐标， 坐标相对于工作区的左上角.<br>
高序单词指定游标的 y 坐标， 坐标相对于工作区的左上角.<br>
...
总结 : `Windows`程序通过`lpfnWndProc`绑定执行窗口过程的函数，在这个函数中处理按键输入消息.
...
`DeferMessage` 做了延迟机制，把输入保存了起来 而不是立即处理，<br>
不管延迟不延迟，最终处理消息的是 `ProcessDeferredMessage`.
...
![](Meta/img2/Slate_SetMesgHandle.png)

```cpp
TSharedRef<FSlateApplication> FSlateApplication::Create(/*...*/)
{
...
实际类型是 `FSlateApplication`.

---

点击一次鼠标 :

![](Meta/img2/MouseInput.png)

输入消息由 `FSlateApplication::RoutePointerUpEvent` 转发到 `ViewPort` .

---

鼠标捕获 `Mouse Capture` 是一种输入机制：当一个窗口或控件捕获了鼠标之后，<br>
所有的鼠标输入事件（移动、按钮按下/抬起、滚轮）都会被直接发送给这个捕获者，无论鼠标光标实际停留在屏幕的哪个位置.

在没有捕获时，鼠标事件会发送给光标正下方的窗口/控件；有捕获时，捕获者会劫持所有后续鼠标事件.

在FPS游戏过程中，玩家希望隐藏系统光标，避免遮挡屏幕.<br>
鼠标移动应该控制视角旋转，而不是鼠标指针到处跑，一不小心点到桌面上去了.<br>
这时就需要鼠标捕获.


所以在转发消息时，if执行的是 `true` 分支 :
```cpp
...
|类|说明|
|---|---|
|FSlateApplication|	Slate 总控，路由输入事件到目标控件.
|SViewport|	Slate 控件，承接鼠标捕获，转发事件到引擎视口.
|FSceneViewport	|引擎视口，转换 Slate 事件为引擎输入事件.
|UGameViewportClient|	游戏视口客户端，分发输入到玩家控制器.
|UCommonGameViewportClient|	可选中间层，添加通用输入处理.
|APlayerController|	玩家控制器，通过输入绑定执行游戏逻辑.

`UCommonGameViewportClient` 是CommonUI的类，使用这个插件时 要把Client设置为这个类.<br>
它继承自 `UGameViewportClient`.


`FViewport`是一个高级抽象接口，用于处理平台特定的渲染、音频和输入子系统.<br>
`GameViewportClient` 是引擎与游戏视口之间的接口.<br>
每个游戏实例会且仅会创建一个 `GameViewportClient`.<br>
到目前为止，唯一可能让一个引擎实例拥有多个游戏实例（从而拥有多个 `GameViewportClient`）的情况是同时运行多个 PIE 窗口.<br>
职责：将输入事件传播到全局交互列表.<br>


类的来历 : 在Init阶段就创建对应的Client.
```cpp
...
`CreateGameWindow`：创建顶层 `SWindow` 游戏主窗口.

`CreateGameViewport`：创建 `SViewport` 控件和 `FSceneViewport`，<br>
并将 `ViewportClient` 的 `Viewport` 指针设置为 `SceneViewport.Get()`.<br>
通过 `GameViewportWidget->SetViewportInterface(SceneViewport.ToSharedRef())` 将 `Slate` 视口与引擎视口关联.


`GameViewportClient` 内部有一个成员 `FViewport* Viewport;` ，
```cpp
...
`SwitchGameWindowToUseGameViewport`：将窗口的内容设置为 `GameViewportWidget`，完成 UI 布局.

输入部分 :

`SetupInitialLocalPlayer` : 最终调用`GameInstance`来创建LocalPlayer.
```cpp
...
`LocalPlayer`的创建与`ViewPortClient`有关，如果是分屏的双人游戏 就会有2个`UPlayer`.
...
ULocalPlayer* UGameInstance::CreateLocalPlayer(/*...*/)
{
	...
	{
		/* bSpawnPlayerController 是 false ，所以这里不会执行，不生成玩家控制器 */
	}
}
```

下图是开始游戏的流程，左侧橙色字体 `GameState` 的下方是创建玩家控制器的地方， 玩家控制器由`ULocalPlayer`创建.

![](Meta/img1/Gameplay.png)

`UWorld::SpawnPlayActor` 调用了 `SetPlayer`，将`UPlayer`传递给玩家控制器.<br>
其中判断了是不是 `ULocalPlayer` ， 如果是的话，初始化输入系统. 如果不是 说明是网络来的，此时 位于服务器上面.
```cpp
...
当 `UGameViewportClient` 接收到一个输入消息时，它会转发给玩家控制器 :
```cpp
bool UGameViewportClient::InputKey(const FInputKeyEventArgs& InEventArgs)
{
	...
	if (TargetPlayer && TargetPlayer->PlayerController)
	{
		bResult = TargetPlayer->PlayerController->InputKey(
			FInputKeyParams(EventArgs.Key, EventArgs.Event, static_cast<double>(EventArgs.AmountDepressed), EventArgs.IsGamepad(), EventArgs.InputDevice));
	}
}
```
从`GEngine`获取 `ULocalPlayer`时，将自己传了进去 ，通过 `ViewPortClient`找到对应`World`的 `GameInstance`，从中获取所有`LocalPlayer`,<br>
之后 将输入消息传给 `LocalPlayer` 的 `PlayerController`.

---

### InputComponent

上一节到了 `SetPlayer` 这一块，输入组件就在这里创建 :
...
以上代码创建了 `UPlayerInput` 和 `UInputComponent`.<br>

`UPlayerInput` 在创建过程中 初始化按键绑定 : 
...
`ForceRebuildingKeyMaps` 从项目设置中获取 `Input` 的配置，并保存下来.

`UPlayerInput` 是一个 只在客户端 `PlayerController` 内部的 `UObject`，它会处理玩家的输入.

`UInputComponent` 是实现用于输入绑定的 `Actor` 组件，允许 `Actor` 将各种形式的输入事件绑定到委托函数上.<br>
由 `PlayerController` 管理的栈来处理，并通过 `PlayerInput` 进行输入处理.<br>
每个绑定都可以消耗输入事件，从而阻止输入栈上的其他组件处理该输入.

---

`SetupInputComponent` : 重写这个函数，在 `InputComponent` 创建之后就可以绑定按键了 :
...
绑定按键就是在构造一个 `FInputActionBinding` ，然后把它塞入 `ActionBindings`.
...
`UInputComponent::AddActionBinding` : 在绑定时 会去找到已经绑定过的那个委托 将它们配对，<br>
例如 下面这个绑定 :<br>
```cpp
InputComponent->BindAction("Scoreboard", IE_Pressed, this, &AShooterPlayerController::OnShowScoreboard);
InputComponent->BindAction("Scoreboard", IE_Released, this, &AShooterPlayerController::OnHideScoreboard);
```
假设已经绑定了 `Pressed`，之后在绑定 `Released`时，根据名称 `Scoreboard` 去找到前面绑定过的那个委托，<br>
把它们的 `bPaired` 设为 `true`.<br>
当其中一个被触发时，可以避免另一个再被意外处理.

---

按键触发 :

![](Meta/img2/PC_ProcessInputStack.png)

关卡蓝图也需要 `InputComponent` :<br>

![](Meta/img2/HighriseLevel.png)

从0到8 分别是 `Pawn` 关卡蓝图 `PlayerController` 的 `InputComponent`.<br>
注意这里是 `Stack`，先进后出.

![](Meta/img2/ProcessPlayerInput_Debug.png)

除了这些输入组件之外，还有一些 `Actor` 可能也需要接收玩家输入，<br>
在蓝图中 把 `AutoReceiveInput` 设为 `Player X` ，<br>
`PreInitializeComponents` 调用 `EnableInput`，把这个`Actor` 自身的 `InputComponent` 传给PC.


收集了这些 `InputComponent` 后，就可以让 `UPlayerInput` 来处理了.

---

**ProcessInputStack**

`PlayerController::PreProcessInput` 默认是空，但可 `override`.
```cpp
...
先执行 `PreProcesInput`，之后执行绑定的按键委托，最后是 `PostProcessInput`.<br>
所以 `Lyra` 是在所有按键委托都执行完以后，才去处理收集到的GA.<br>

`PreProcessInput` 什么都没做.
```cpp
...
`UPlayerInput` 从项目设置中获取按键绑定，但终究是保存在这几个变量中，<br>
那么就意味着可以在运行时修改这些变量，从而达到使用资产来配置按键绑定，甚至动态修改绑定.
...
```cpp
EBlackboardNotificationResult UBTDecorator_Blackboard::OnBlackboardKeyValueChange(
...
`bPass`：表示装饰器条件当前是否满足，即 `IsSet`是否为`true`
...
|---|---|---|---|
|false|false|yes|分支未执行，且条件不满足 → 正常，无需干预.|
|false|true|no|分支未执行，但条件已满足 → 应该进入该分支.|
|true|false|no|分支正在执行，但条件不再满足 → 应该退出该分支.|
|true|true|yes|分支正在执行，且条件满足 → 正常，但 ValueChange 模式下仍可能因值变化而强制重启.|
 ...
 *
 *	Fast TArray Replication 是 NetDeltaSerialize 的一种自定义实现，适用于 UStruct 的 TArray.它为大数据集提供了性能改进，
 *	能够最优地序列化数组中任意位置的删除操作，并允许在客户端对添加和删除操作触发事件.其缺点是，你需要编写游戏代码来标记数组中变动的条目，
 *	并且在某些情况下，客户端和服务器的列表*顺序*不能保证完全相同.
 *
 *	使用 FTR 更为复杂，但你需要按以下步骤编写代码：
 *
 */
#if 0
	
/** 步骤 1：让你的结构体继承自 FFastArraySerializerItem */
USTRUCT()
struct FExampleItemEntry : public FFastArraySerializerItem
{
	...
	/** 
	 * 可选函数，你可以在客户端实现以接收条目变更的通知；
	 * 参数类型可以与 FastArrayDeltaSerialize 调用中第二个模板参数的类型相匹配.
	 * 
	 * 注意：在这些函数内部修改数组序列化器的内容是不安全的，也不能假定数组内容已完全更新，
	 * 因为这些函数是在条目逐个更新时调用的，因此可能在批量更新的中途被调用.
	 */
	...
	// 可选：用于 LogNetFastTArray 日志记录的调试字符串
	FString GetDebugString();

};

/** 步骤 2：你必须将 TArray 包装在另一个继承自 FFastArraySerializer 的结构体中 */
USTRUCT()
struct FExampleArray: public FFastArraySerializer
{
	...
	UPROPERTY()
	TArray<FExampleItemEntry>	Items;	/** 步骤 3：你必须有一个名为 Items 的 TArray，其元素类型是你在步骤 1 中创建的结构体. */

	/** 步骤 4：复制这段代码，将 example 替换为你的名称 */
...
/** 步骤 5：复制并粘贴此结构体特征特化，将 FExampleArray 替换为你在步骤 2 中创建的结构体. */
template<>
...
/** 步骤 6 及以后： 
 *		- 声明一个你创建的 FExampleArray（步骤 2）类型的 UPROPERTY.
 *		- 当你更改数组中的某个条目时，必须调用 FExampleArray 的 MarkItemDirty 函数.你需要传入被修改的条目的引用.
 *			请参阅 FFastArraySerializer::MarkItemDirty.
 *		- 如果你从数组中移除了某个条目，必须调用 FExampleArray 的 MarkArrayDirty.
 *		- 在你的类的 GetLifetimeReplicatedProps 中，使用 DOREPLIFETIME(YourClass, YourArrayStructPropertyName);
 *
 *		你可以在你的结构体（步骤 1）中提供以下函数，以在添加/删除/变更之前获得通知：
...
`FFastArraySerializerItem` 和 `FFastArraySerializer` 都有Remove Add Change函数，<br>
函数调用顺序 Remove -> Add ->Change :<br>

整体调用顺序 ：
```cpp
// 1. 先通知每一个被删除的条目（条目级回调）
for (int32 idx : Header.DeletedIndices)
{
    if (Items.IsValidIndex(idx))
    {
        Items[idx].PreReplicatedRemove(ArraySerializer);  // ← "elements themselves are notified"
    }
}
...
```
Item `FGameplayAbilitySpec` 定义了Remove、Add操作，<br>
如果能触发这两个函数，就意味着这个Item已经序列化在内存中 数据是有效的，<br>
所以 `*this`是合法的.
...
`ShouldWriteFastArrayItem` 决定某个数组条目是否应该参与网络复制.<br>
`bIsWritingOnClient` : <br>
当客户端作为记录者（例如网络回放或客户端预测）需要序列化数组时,<br>
拥有 有效 `ReplicationID`（即该条目是从服务器正式同步过来的）的条目才会被写入.<br>
客户端本地预测添加的条目（ReplicationID == INDEX_NONE）会被忽略，避免污染回放数据或产生不一致.<br>
 ...
 *	===================== 网络序列化概述及其工作原理 =====================
 *
 *		一切源于 UNetDriver::ServerReplicateActors.
 *		选择要复制的 Actor，创建 Actor 通道，然后调用 UActorChannel::ReplicateActor.
 *		ReplicateActor 最终负责决定哪些属性已更改，并构造一个 FOutBunch 发送给客户端.
 *
 *	UActorChannel 有两种方式来决定需要发送哪些属性.
 *		传统方式：一个扁平的 TArray<uint8> 缓冲区：UActorChannel::Recent.这代表了该 Actor 属性的一个扁平内存块.
 *		如果你知道 FProperty 的偏移量，这个内存块可以逐字转换为 AActor* 并查找属性值.
 *		Recent 缓冲区表示使用该 Actor 通道的客户端所拥有的值.我们使用 Recent 与当前值进行比较，以决定发送什么.
 *
 *		这对于“原子”属性（如 int、float、object* 等）非常有效.
 *		但对于“动态”属性（如 TArray，它们存储 Num/Max 以及指向数组数据的指针）则不适用.
 *		数组数据在扁平的 ::Recent 缓冲区中没有存储空间.（对于这些属性，“动态”可能不是一个好名字）
 *
 *		为了解决这个问题，UActorChannel 还为“动态”状态维护了一个 TMap：UActorChannel::RecentDynamicState.
 *		这个映射允许我们根据属性的 RepIndex 来查找属性的“基准状态”.
 *
 *	NetSerialize 与 NetDeltaSerialize
 *		适合放入扁平 Recent 缓冲区的属性可以完全通过 NetSerialize 进行序列化.NetSerialize 只是向 FArchive 读取或写入数据.
 *		由于复制系统可以直接查看 Recent[] 缓冲区并进行直接比较，它可以判断哪些属性变脏了.NetSerialize 仅仅负责读写.
 *
 *		动态属性只能通过 NetDeltaSerialize 进行序列化.NetDeltaSerialize 是基于给定的基准状态进行的序列化，
 *		它会生成一个“增量”状态（发送给客户端）和一个“完整”状态（保存下来，作为将来增量序列化时的基准状态）.
 *		NetDeltaSerialize 本质上既要进行差异比较，又要进行序列化.它必须进行差异比较，以便知道需要发送属性的哪些部分.
 *	
 *	基准状态与动态属性复制.
 *		对于复制系统 / UActorChannel 而言，基准状态可以是任何东西.基准状态仅处理 INetDeltaBaseState*.
 *
 *		UActorChannel::ReplicateActor 最终将决定是调用 FProperty::NetSerializeItem 还是 FProperty::NetDeltaSerializeItem.
 *
 *		如上所述，NetDeltaSerialize 接受一个额外的基准状态，并产生一个差异状态和一个完整状态.生成的完整状态将被用作未来增量序列化的基准状态.
 *		NetDeltaSerialize 使用基准状态和 Actor 的当前值来决定需要发送哪些部分.
 *		
 *		INetDeltaBaseState 在 NetDeltaSerialize 函数内部创建.复制系统 / UActorChannel 不了解其内部细节.
 *
 *		目前，有两种形式的增量序列化：通用复制和快速数组复制.
 *
 *	
 *	通用增量复制
 *		通用增量复制通过 FStructProperty::NetDeltaSerializeItem、FArrayProperty::NetDeltaSerializeItem、FProperty::NetDeltaSerializeItem 实现.
 *		其工作原理是首先对对象的当前状态进行 NetSerialize（“完整”状态），然后使用 memcmp 将其与之前的基准状态进行比较.
 *		FProperty 实际上负责实现比较，如果当前状态已更改，则将其写入差异状态，并且始终写入完整状态.
 *		FStructProperty 和 FArrayProperty 函数通过迭代其字段或数组元素，并调用 FProperty 函数来实现，同时嵌入元数据.
 *
 *		例如，FArrayProperty 基本会写入：
 *			"数组现在有 X 个元素" -> "这是元素 Y" -> FProperty::NetDeltaSerialize 的输出 -> "这是元素 Z" -> 等等
 *
 *		通用数据复制是处理 FArrayProperty 和 FStructProperty 序列化的“默认”方式.只要子属性能够进行 NetSerialize，
 *		这种方式适用于任何数组或结构体.
 *
 *	自定义网络增量序列化
 *		自定义网络增量序列化通过结构体特征系统工作.如果一个结构体具有 WithNetDeltaSerializer 特征，
 *		那么其原生的 NetDeltaSerialize 函数将被调用，而不会走 FStructProperty::NetDeltaSerializeItem 中的通用增量复制代码路径.
 *
 *	Fast TArray 复制
 *		Fast TArray 复制通过自定义网络增量序列化实现.它不关心表示状态的扁平 TArray 缓冲区，
 *		而只关心 ID 和 ReplicationKey 的 TMap.这些 ID 映射到数组中的条目，这些条目都在 FFastArraySerializerItem 中定义了 ReplicationID 字段.
 *		FFastArraySerializerItem 还有一个 ReplicationKey 字段.当通过 MarkItemDirty 标记条目为脏时，
 *		它们会被赋予一个新的 ReplicationKey，如果它们还没有 ReplicationID，则会被分配一个新的 ReplicationID.
 *
 *		FastArrayDeltaSerialize（定义如下）
 *		在服务器序列化（写入）期间，我们将旧的基准状态（例如，旧的 ID<->Key 映射）与数组的当前状态进行比较.
 *		如果有条目缺失，我们将它们在 bunch 中作为删除写入.如果它们是新的或已更改，它们将作为已更改写入，并通过 NetSerialize 调用序列化其状态.
 *
 *		例如，实际写入的内容可能看起来像：
 *			"数组有 X 个已更改元素，Y 个已删除元素" -> "元素 A 已更改" -> 结构体条目其余部分的 NetSerialize 输出 -> "元素 B 已删除" -> 等等
 *
 *		请注意，ReplicationID 在客户端和服务器之间是复制并保持同步的.索引则不同步.
 *
 *		在客户端序列化（读取）期间，客户端读取已更改元素的数量和已删除元素的数量.它还会构建一个 ReplicationID 到当前数组本地索引的映射.
 *		在反序列化 ID 时，它会查找该元素，然后执行所需操作（必要时创建、反序列化当前状态或删除）.
 *
 *		内部结构体的增量序列化现在默认启用.这意味着当 ReplicationKey 发生变化时，我们会将结构体的当前状态与最后发送的状态进行比较，
 *		跟踪变更列表，并仅发送已更改的属性，这与标准复制路径完全相同.
 *		如果这对特定的 FastArray 类型造成问题，可以在构造函数中调用 FFastArraySerializer::SetDeltaSerializationEnabled(false) 来禁用它.
 *		该功能可以通过将 "net.SupportFastArrayDelta" CVar 设置为 0 来完全禁用.
 *
 *		ReplicationID 和 ReplicationKey 由 FFastArraySerializer 上的 MarkItemDirty 函数设置.它们只是 int32，按顺序分配.
 *		除了唯一性之外，它们没有什么特别之处.
 */
```
...
例如 客户端执行的技能链是 X技能-->Y技能--->Z技能， 当X技能触发Y技能时，函数堆栈大概是这样的 :
...
在GA内部触发的GE 可以绑定预测，而直接调用ASC的`ApplyGameplayEffect`等函数 就不会预测了， 一般来说 这样的GE应该由服务器来执行，<br>
...
在初始化组件时，把Actor的RootComponent设为 `UpdatedComponent`.<br>

在`Character`类中， `UpdatedComponent`就是胶囊体.

此后所有移动：<br>
`PerformMovement` → `SafeMoveUpdatedComponent` → `UpdatedComponent->MoveComponent`<br>
都会操作这个胶囊体，而角色的骨骼网格体会跟随胶囊体移动.
...
`UpdatedComponent`是胶囊体，而胶囊体组件又从`UPrimitiveComponent`继承下来，
`UPrimitiveComponent`重写了`IsSimulatingPhysics`函数，所以 如果启动了胶囊体的物理模拟，移动组件就不去更新胶囊体的位置了.
...
void AMyPlayerController::Move(const FInputActionValue& Value)
{
	...
	GetCharacter()->AddMovementInput(RightVector,InputValue.X);
	GetCharacter()->AddMovementInput(ForwardVector,InputValue.Y);
}
...
```
`AddMovementInput` 最终调用到了 `Internal_AddMovementInput` ，把玩家控制器输入进来的Vector保存下来.
...
```
`InputVector` 就是从玩家控制器那里传来的Vector.

`ConsumeInputVector` 顾名思义，把玩家控制器传来的Vector拿走，一个东西被拿走后 原地就剩下0个东西.

在 `ControlledCharacterMove` 中，如果是权威 直接执行 `PerformMovement`.<br>
如果是客户端 或者 自主代理，调用 `ReplicateMoveToServer` ，在这个函数内部 依然执行了 `PerformMovement`.

```cpp
void UCharacterMovementComponent::ControlledCharacterMove(const FVector& InputVector, float DeltaSeconds)
{
    // InputVector 是通过 ConsumeInputVector() 获得的当前帧累计输入
    CharacterOwner->CheckJumpInput(DeltaSeconds);
    Acceleration = ScaleInputAcceleration(ConstrainInputAcceleration(InputVector));
    AnalogInputModifier = ComputeAnalogInputModifier();
    // ... 然后调用 ReplicateMoveToServer (客户端) 或 PerformMovement (服务器)
}
```

`ConstrainInputAcceleration` : 根据当前状态对输入进行限制.例如，如果正在行走且向上看，则不要向上移动.

`ScaleInputAcceleration`：将 `InputVector` 乘以 `MaxAcceleration`，得到最终的 `Acceleration` 向量.<br>
例如 `MaxAcceleration` 默认为 `2048 cm/s²`，那么满输入时加速度为 `2048 * 方向`.
...
角色可以站在其他物体之上，例如一个移动的电梯平台，<br>
平台会每帧改变自己的位置和旋转，角色必须跟随平台运动，才能保持“站在上面”的视觉效果和碰撞交互.<br>
但是，角色的移动不应该简单地每帧被瞬间拉到平台上，那样会忽略碰撞，可能卡入其他物体.<br>
应该以 sweep 方式移动，检测是否被障碍物阻挡，同时还要考虑平台的旋转对角色朝向的影响.

`MaybeUpdateBasedMovement` 使角色能够平滑、安全地跟随移动的平台.
...
void UCharacterMovementComponent::MaybeUpdateBasedMovement(float DeltaSeconds)
{
	...
	// 对于Movable物体 返回true，静态物体返回false.
    if (MovementBaseUtility::UseRelativeLocation(MovementBase))
    {
        if (!MovementBaseUtility::IsSimulatedBase(MovementBase))
        {
            // 非模拟物理的基底：立即更新位置
			UpdateBasedMovement();
            // 如果之前启用了后物理 tick，现在关闭它，恢复普通 tick 依赖
        }
        else
        {
            // 模拟物理的基底：延迟更新到物理后
...
移动组件通过 `ACharacter::SetBase` 给角色设置地面， 通过 `GetMovementBase` 获取地面组件.

非模拟物理基底：例如 手动控制移动的电梯（Mobility 可移动，但不模拟物理）,<br>
这类基底的运动是确定性的、可预测的，角色可以立即跟随.<br>

模拟物理基底：例如 受物理引擎驱动的载具、被推动的箱子,<br>
这类基底的位置、旋转在物理模拟后才能最终确定，因此角色必须等到物理阶段完成后再跟随，否则会导致位置不一致.

`UpdateBasedMovement` :<br>
获取基底的当前变换<br>
计算基底相对于上一帧的位置旋转变化<br>
根据基底的移动，计算角色应该移动的向量， 例如站在电梯上 相对位置保持不变，<br>
使用 `MoveUpdatedComponent` 移动角色，更新角色朝向.
...
```
`ApplyAccumulatedForces` 检查要应用的力，如果Z方向的力 > 0 就把移动模式设为 `Falling`， 然后把力叠加在 `Velocity` 上.

`UpdateCharacterStateBeforeMovement` 根据是否蹲下，调整胶囊体的高度.

`StartNewPhysics` 就是执行各个移动模式的地方，例如 Walking Flying ...

---

![](Meta/img2/CMC_WalkStack.png)

`PhysWalking` 内部有一个大while，大概流程是这样的:<br>
保存移动前的状态，维持水平速度 保证Z轴速度是0，<br>
计算加速度和速度，计算移动增量并执行移动 - `MoveAlongFloor`，<br>
更新地面信息，处理边缘掉落，<br>
处理可走地面:<br>
- 如果是可走地面，调整高度使其略高于地面 避免陷入，并设置SetBase.
- 如果不可走且没有穿透，则尝试开始下落.
...
`Plugins/Experimental/GameplayTargetingSystem`

使用射线检测等手段 检测场景中的敌人，通常需要写大一串节点，有时候还要按照条件去过滤掉一些目标.<br>
在不同的技能中，将这些节点复制了一遍又一遍，非常繁琐.

`TargetingSystem` 把这套流程封装了起来，将流程定义为可配置的资产数据，把每个步骤拆分到不同的 `UObject` 中，可以自由组合以及扩展.

`Task` 分为3种类型:<br>
选择: 射线检测等 获得Actor的手段.<br>
过滤: 筛选出那些满足特定条件的Actor.<br>
排序: 可以按照距离排出优先级.

在 `UTargetingPreset` 配置这些Task，
```cpp
USTRUCT(BlueprintType)
...
`Task` 被添加在一个 `TArray` 中，执行时 从第一个 `Task` 开始执行. <br>
所以 添加的顺序非常重要.
...
整个系统主要由 `UTargetingSubsystem` 驱动，`Task`的输出结果保存在一个全局变量中.<br>
当 `Task` 执行完毕后，通过一个 `ID` 去全局变量里找对应的数据.

这样做的好处是 各个 `Task` 之间不会互相依赖，把输出结果保存在公共区域，可以让下一个`Task`通过ID来读取数据.

---

`AbilityTask_PerformTargeting::Activate`

这是公开的异步节点，要在GA里使用这个系统 就是从这里启动.
...
`MakeTargetRequestHandle` 把 `TargetingPreset`里面的`Task` 和 `SourceContext` 保存在全局中，接着 返回一个ID.

反过来，通过这个ID 可以到全局中查出对应的 `TargetingPreset` 和 `SourceContext`.<br>
这个 ID ( `TargetingHandle` ) 贯穿本次执行的主线.

由于整个系统是使用全局数据实现的，所以 `bReleaseOnCompletion` 保证了执行完以后清理全局中的数据.
...
`ExecuteTargetingRequestWithHandleInternal` 就是真正的执行过程.

流程也很简单， 从全局中找到并执行 之前保存的 `Task`， <br>
执行完了 广播一下，触发异步节点的 `OnTargetReady` 引脚，蓝图里就可以通过节点返回的`TargetingHandle` 去全局中获取数据了. <br>

蓝图里的内容执行完以后，`ReleaseTargetRequestHandle` 将会清理这个流程的数据， 全局变量会根据ID 移除这一次保存的数据.
...
`PlayerCameraManager` 负责管理特定玩家的摄像机.<br>
它定义了最终被其他系统（例如渲染器）所使用的视图属性，你可以将其视为你在世界中的虚拟眼瞳.<br>

它可以直接计算最终的摄像机属性，也可以在影响摄像机的其他对象或角色之间进行仲裁/混合（例如从一个CameraActor混合到另一个）.<br>

`PlayerCameraManager` 主要的外部职责是可靠地响应各种 `Get*()` 函数，例如 `GetCameraViewPoint`.<br>
除此之外的大部分内容都是实现细节，可以由用户项目覆盖.<br>

默认情况下，`PlayerCameraManager` 维护一个 `ViewTarget`，这是摄像机关联的主要角色.<br>
它还可以对最终的视图状态应用各种“后期”效果，例如摄像机动画、晃动、后处理效果或特殊效果（如镜头污迹）.<br>


更新时机 : 在Actor Tick之后，网络Tick之前.
```cpp
void UWorld::Tick( ELevelTick TickType, float DeltaSeconds )
{
	/*...*/
...
`FTViewTarget ViewTarget` 这个结构体中保存了一个Actor，<br>
这个Actor可以是ACameraActor，也可以是玩家角色，它们都是Camera组件的拥有者.

`UpdateViewTarget` : 根据不同的 `CameraStyle` 执行对应的计算方式，计算出新的位置和旋转.<br>
`CameraStyle` 的默认值是 `NAME_Default`.
```cpp
...
```
所以默认会到 `UpdateViewTargetInternal` 这一条支线来计算摄像机的数据，位置和旋转什么的.

`BlueprintUpdateCamera` 可以在`PlayerCameraManager`的蓝图类里面实现这个函数，做自定义的计算.

默认执行对应Actor的 `CalcCamera`.
```cpp
...
`GetCameraView` 返回了一系列参数. 

回到梦开始的地方，经过层层访问，`ViewTarget` 保存了摄像机组件的数据.
```cpp
void APlayerCameraManager::DoUpdateCamera(float DeltaTime)
{
	ViewTarget.CheckViewTarget(PCOwner);
...
`CameraCachePrivate` 保存了摄像机数据 `POV` 和 `TimeStamp`，<br>
为什么要单独搞一个变量放这些数据？ 甚至还有一个专门的Get函数 :
...
经过查找， 这和 `ULocalPlayer` 有关， 在前文的 `输入系统` 这一节 发现 `ULocalPlayer` 和 `GameViewportClient` 有关，<br>
也许这就是获取渲染数据的地方 :
...
`ULocalPlayer::CalcSceneView` `ULocalPlayer::GetProjectionData` 都调用了 `GetViewPoint` 这个函数，

`GetProjectionData` 有一个用途就是向蓝图公开一个函数，用于转换屏幕和世界坐标.

`CalcSceneView` 就有点重量级了，它在 `UGameViewportClient::Draw` 函数里被调用.

这一切的源头都来自于 `Engine` : 根据注释 更加印证了这就是获取渲染所需数据的途径，最终从摄像机组件返回数据.
